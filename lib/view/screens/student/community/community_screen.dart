...
class CommunityScreen extends StatefulWidget {
  ...
  const CommunityScreen({this.isAdmin = false, Key? key}) : super(key: key);

  @override
  State<CommunityScreen> createState() => _ComplainScreenState();
}

class _ComplainScreenState extends State<CommunityScreen> {
  ...
  void initState() {
    ...
    super.initState();
    Provider.of<CommunityProvider>(context, listen: false).getAllCommunity();
    controller.addListener(() {
      if (controller.offset >= controller.position.maxScrollExtent &&
          !controller.position.outOfRange &&
          Provider.of<CommunityProvider>(context, listen: false).hasNextData) {
        Provider.of<CommunityProvider>(context, listen: false).updateAllCommunity();
      }
    });
  }

  Future<void> _refresh(BuildContext context) async {
    Provider.of<CommunityProvider>(context, listen: false).getAllCommunity(isFirstTime: false);
  }
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      ...
      child: Scaffold(
          ...
          appBar: const CustomAppBar(title: 'Community', borderRadius: 0, isOpenCommunity: true),
